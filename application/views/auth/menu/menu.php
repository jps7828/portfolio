						...
						<div class="card-body">
							<div class="table-responsive">
								<table class="table table-hover text-nowrap text-md-nowrap table-striped mg-b-0s">
									...
									<tbody>
										<?php $i=1; foreach ($menu_view as $row): ?>
										<tr>
											<td><?= $i++ ;?></td>
											<td><?= $row->menu_name; ?></td>
											<td><?= $row->menu_type; ?></td>
											<td><?= $row->sub_menu_of; ?></td>
											<td><?= $row->menu_ordering; ?></td>
											<td><?= $row->menu_status; ?></td>
											<td> 
												<a href="javascript:;" onclick="edit('<?= $row->menu_id ?>')" class="btn btn-outline-info btn-sm rounded-11 mr-2"> <i class="fe fe-edit-2"></i> </a>
												<a href="javascript:;" onclick="delete_data('<?= $row->menu_id?>')" class="btn btn-outline-danger btn-sm rounded-11 mr-2"> <i class="fe fe-trash-2"></i> </a>
											</td>
										</tr>
									<?php endforeach ?>
								</tbody>

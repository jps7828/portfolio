						...
						<div class="card-body">
							<div class="table-responsive">
								<table class="table table-hover text-nowrap text-md-nowrap table-striped mg-b-0s">
									...
									<tbody>
										<?php $i=1; foreach ($page_view as $row): ?>
										<tr>
											<td><?= $i++ ;?></td>
											<td><?= $row->page_name; ?></td>
											<td><?= $row->page_used_for; ?></td>
											<td><?= $row->page_status; ?></td>
											<td> 
												<a href="javascript:;" onclick="edit('<?= $row->page_id ?>')" class="btn btn-outline-info btn-sm rounded-11 mr-2"> <i class="fe fe-edit-2"></i> </a>
												<a href="javascript:;" onclick="delete_data('<?= $row->page_id?>')" class="btn btn-outline-danger btn-sm rounded-11 mr-2"> <i class="fe fe-trash-2"></i> </a>
											</td>
										</tr>
									<?php endforeach ?>
								</tbody>

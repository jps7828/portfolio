						...
						<div class="card-body">
							<div class="table-responsive">
								<table class="table table-hover text-nowrap text-md-nowrap table-striped mg-b-0s">
									...
									<tbody>
										<?php $i=1; foreach ($contact_us_view as $row): ?>
										<tr>
											<td><?= $i++ ;?></td>
											<td><?= $row->contact_us_phone; ?></td>
											<td><?= $row->contact_us_phone_2; ?></td>
											<td><?= $row->contact_us_email; ?></td>
											<td><?= $row->contact_us_status; ?></td>
											<td> 
												<a href="javascript:;" onclick="edit('<?= $row->contact_us_id ?>')" class="btn btn-outline-info btn-sm rounded-11 mr-2"> <i class="fe fe-edit-2"></i> </a>
												<a href="javascript:;" onclick="delete_data('<?= $row->contact_us_id?>')" class="btn btn-outline-danger btn-sm rounded-11 mr-2"> <i class="fe fe-trash-2"></i> </a>
											</td>
										</tr>
									<?php endforeach ?>
								</tbody>

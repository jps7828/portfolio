								...
								<table class="table table-hover text-nowrap text-md-nowrap table-striped mg-b-0s">
									...
									<tbody>
										...
										<tr>
											...
											<td><?= substr($row->portfolio_heading, 0,50) ; ?>...</td>
											<td><?= $row->portfolio_ordering; ?></td>
											<td><?= $row->portfolio_status; ?></td>
											<td> 
												<a href="javascript:;" onclick="edit('<?= $row->portfolio_id ?>')" class="btn btn-outline-info btn-sm rounded-11 mr-2"> <i class="fe fe-edit-2"></i> </a>
												<a href="javascript:;" onclick="delete_data('<?= $row->portfolio_id?>')" class="btn btn-outline-danger btn-sm rounded-11 mr-2"> <i class="fe fe-trash-2"></i> </a>
											</td>
										</tr>
									<?php endforeach ?>
								</tbody>
